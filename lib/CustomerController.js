...
module.exports = function (common) {
    ...
    this.addProperty = (objectName, data)=>{
        ...
        return this.hubspotClient.crm.properties.batchApi.create( this.type==="companies"? "company": "contact", {
                     inputs: data
             }).then((res)=>{
                     cb(null, res.body)
                 }).catch(cb)
    ...
    this.get = (limit, after, properties, id)=>{
        if(id){
            return this.api.get()
        }else{
            return this.api.getAll(limit, after, properties)
        }
